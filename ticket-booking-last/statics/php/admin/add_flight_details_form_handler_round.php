	...
	<body>
		<?php
			if(isset($_POST['Submit']))
			{
				...
				if(empty($_POST['ngay_ve']))
				{
					$data_missing[]='Ngay Ve';
				}
				else
				{
				...
				if(empty($data_missing))
				{
					...
					if($cnt==1)
					{
						$query="INSERT INTO chuyen_bay (ma_chuyen_bay,from_city,to_city,ngay_bat_dau,ngay_ket_thuc,gio_bat_dau,gio_ket_thuc,so_luong_economy,so_luong_business,gia_economy,gia_business,ma_may_bay,ngay_ve)
						 VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)";
						$stmt=mysqli_prepare($dbc,$query);
						mysqli_stmt_bind_param($stmt,"sssssssiiiiss",$flight_no,$origin,$destination,$dep_date,$arr_date,$dep_time,$arr_time,$seats_eco,$seats_bus,$price_eco,$price_bus,$jet_id,$ngay_ve);
						mysqli_stmt_execute($stmt);
					...
					if($affected_rows==1)
					{
						...
						header("location: ../../html/admin/add-flight_khuhoi.php?msg=success");
					}
					else
					{
						echo "Thêm Thất Bại";
						echo mysqli_error();
						header("location: ../../html/admin/add-flight_khuhoi.php?msg=failed");
					}
				}
				else
				{
