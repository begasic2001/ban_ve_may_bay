	...
	<body>
		<?php
			if(isset($_POST['Pay_Now']))
			{
				...
				if($class=='economy')
				{
					$query="UPDATE chuyen_bay SET so_luong_economy=so_luong_economy-? WHERE ma_chuyen_bay=? AND ngay_bat_dau=? and ngay_ve=?";
					$stmt=mysqli_prepare($dbc,$query);
					mysqli_stmt_bind_param($stmt,"isss",$no_of_pass,$flight_no,$journey_date,$ngay_ve);
				...
				{
					$query="UPDATE chuyen_bay SET so_luong_business=so_luong_business-? WHERE ma_chuyen_bay=? AND ngay_bat_dau=? and ngay_ve=?";
					$stmt=mysqli_prepare($dbc,$query);
					mysqli_stmt_bind_param($stmt,"isss",$no_of_pass,$flight_no,$journey_date,$ngay_ve);
					mysqli_stmt_execute($stmt);
					$affected_rows_1=mysqli_stmt_affected_rows($stmt);
				...
				if($affected_rows_1==1)
				{
					...
					if($affected_rows_2==1)
					{
						...
						header('location:../../html/user/ticket-success_khuhoi.php');
					}
					else
					{
						echo "Submit Error";
