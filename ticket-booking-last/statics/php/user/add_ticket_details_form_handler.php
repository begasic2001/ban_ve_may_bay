...
<body>
	<?php
		...
		if(isset($_POST['Submit'])) {
			...
			$query = 
				"INSERT INTO ve 
				(code_ve,ngay_dat_ve,ma_chuyen_bay,ngay_bat_dau,loai,trang_thai_dat,ma_khach_bay,lui_phong_cho,dang_ky_uu_tien,bao_hiem,ma_hoa_don,ma_khach_mua) 
				VALUES (?,?,?,?,?,?,?,?,?,?,?,?)";
			$stmt = mysqli_prepare($dbc, $query);
			...
			for($i=1; $i <= $no_of_pass; $i++) {
				...
				$query = 
					...
					(ma_khach_bay,code_ve,ten_khach_bay,tuoi,gioi_tinh,chon_bua_an,ma_so) 
					VALUES (?,?,?,?,?,?,?)";
				$stmt=mysqli_prepare($dbc, $query);
			...
			header("location: ../../html/user/confirm_third.php");
		} else {
